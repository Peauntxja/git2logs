...
from gui.styles import UIStyles, resource_path, get_script_path, _resolve_monospace_font
from gui.service_bridge import ServiceBridgeMixin

class Git2LogsGUI(ServiceBridgeMixin):
    ...
    def _perform_excel_export(self, template: str, output: str, selected_projects: list | None) -> None:
        ...
        try:
            self.log("=" * 60, "info")
            excel_params = self._build_excel_params(template, output, selected_projects)
            saved_path = self._service.export_excel(excel_params, self._service_log_callback)
            self._show_toast("Excel 导出成功", "success")
